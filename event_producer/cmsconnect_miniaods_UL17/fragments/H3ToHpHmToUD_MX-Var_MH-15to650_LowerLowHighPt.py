...
def pset(mx, mh, weight):
    return cms.PSet(
        ...
        ConfigDescription = cms.string('H3ToHpHmToUD_MX%.0f_MH%.0f' % (mx, mh)),
        PythiaParameters = cms.PSet(
            ...
            processParameters = cms.vstring(
                '37:onMode = off',
                '37:oneChannel = 1 1.000000 100 -1 2', # H+ -> u d
...
for mh in low_m_higgs:
    ...
    for mx in low_m_res:
        print('H3ToHpHmToUD_MX%.0f_MH%.0f weight %.4f' % (mx, mh, weight))
    ...
    for mx in mres_lowpt_points(mh, n=len(low_m_res)):
        print('H3ToHpHmToUD_MX%.0f_MH%.0f weight %.4f' % (mx, mh, 0.5*weight))
