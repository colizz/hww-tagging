...
def pset(mx, mh, weight):
    return cms.PSet(
        ...
        GridpackPath =  cms.string('instMG://H3ToHpHm_2HDM/MG5_aMC_v2.6.5/%.0f:%.0f' % (mx, mh)),
        ConfigDescription = cms.string('H3ToHpHmToCSorBC_MX%.0f_MH%.0f' % (mx, mh)),
        PythiaParameters = cms.PSet(
            ...
            processParameters = cms.vstring(
                '37:onMode = off',
                '37:oneChannel = 1 0.500000 100 -3 4', # H+ -> c s
                '37:addChannel = 1 0.500000 100 -5 4', # H+ -> b c
...
# append low-mass points (low pT + lower pT)
for mh in low_m_higgs:
    ...
    # for mx in low_m_res:
    #     print('H3ToHpHmToCSorBC_MX%.0f_MH%.0f weight %.4f' % (mx, mh, weight))
    ...
    for mx in mres_lowpt_points(mh, n=len(low_m_res)):
        print('H3ToHpHmToCSorBC_MX%.0f_MH%.0f weight %.4f' % (mx, mh, 0.5*weight))
        generator.RandomizedParameters.append(pset(mx, mh, 0.5*weight))
