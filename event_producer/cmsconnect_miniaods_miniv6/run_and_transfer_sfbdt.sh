...
# 2. run NanoAOD step after MiniAOD
# 3. for both MiniAOD and NanoAOD steps: customize commands to keep all gen particles
# 4. for NanoAOD step: small tweaks on genWeights
...
export RELEASE_SKIM=CMSSW_15_0_4
...
## custom setup to keep all gen particles
CUSTOMCMD_MINIAOD='process.prunedGenParticles.select = cms.vstring("keep *"); process.prunedGenParticlesWithStatusOne.select = cms.vstring("keep *")'
CUSTOMCMD_NANOAOD='process.finalGenParticles.select = cms.vstring("keep *"); process.genWeightsTable.allowedNumScaleWeights = cms.vuint32(0)'

# Run MiniAODv6
cmsDriver.py  --python_filename MiniAODv6_cfg.py --eventcontent MINIAODSIM --customise Configuration/DataProcessing/Utils.addMonitoring --datatier MINIAODSIM --fileout file:miniv6.root --conditions $GLOBALTAG_SKIM --customise_commands "$CUSTOMCMD_MINIAOD" --step PAT --geometry DB:Extended --filein file:reco.root --era Run3_2024 --mc --nThreads $NTHREAD -n $NEVENT || exit $? ;

# Run NanoAODv15
cmsDriver.py  --python_filename NanoAODv15_cfg.py --eventcontent NANOAODSIM1 --scenario pp --customise Configuration/DataProcessing/Utils.addMonitoring --datatier NANOAODSIM --fileout file:nanov15.root --conditions $GLOBALTAG_SKIM --customise_commands "$CUSTOMCMD_NANOAOD" --step NANO --filein file:miniv6.root --era Run3_2024 --mc --nThreads $NTHREAD -n $NEVENT || exit $? ;

# Transfer file
xrdcp --silent -p -f nanov15.root $EOSPATH
touch dummy.cc
