...
if [ "$WORKAREA" == "local" ]; then
  ...
  cd $CMSSWDIR/CMSSW_15_0_0_pre3/src
  eval `scram runtime -sh`
else
  CMSSWDIR="."
  scram p CMSSW CMSSW_15_0_0_pre3
...
for infile in "${ADDR[@]}"; do
  echo $infile $idx
  retry cmsRun $CMSSWDIR/CMSSW_15_0_0_pre3/src/DeepNTuples/Ntupler/test/DeepNtuplizerAK8Scout.py inputFiles=${infile} ${CMSRUNARGS}
