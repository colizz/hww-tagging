...
mv hww-tagging-dev-miniaods/event_producer/cmsconnect_miniaods_Run3_haa/{inputs,fragments} .
mv hww-tagging-dev-miniaods/event_producer/cmsconnect_miniaods_Run3/inputs/{model,InputCards} inputs
...
export RELEASE=CMSSW_12_4_11_patch3
...
GLOBALTAG_SKIM=124X_mcRun3_2022_realistic_postEE_v1
...
# cmsDriver.py Configuration/GenProduction/python/${PROCNAME}.py --python_filename GS_cfg.py --eventcontent RAWSIM,LHE --customise Configuration/DataProcessing/Utils.addMonitoring --datatier GEN-SIM,LHE --fileout file:gensim.root --conditions $GLOBALTAG --beamspot DBrealistic --customise_commands process.RandomNumberGeneratorService.generator.initialSeed="int(${SEED})"\\nprocess.source.numberEventsInLuminosityBlock="cms.untracked.uint32(${NEVENTLUMIBLOCK})" --step LHE,GEN,SIM --geometry DB:Extended --era Run3_2024 --mc --nThreads $NTHREAD -n $NEVENT || exit $? ;

cmsDriver.py Configuration/GenProduction/python/${PROCNAME}.py --era Run3 --customise Configuration/DataProcessing/Utils.addMonitoring --beamspot Realistic25ns13p6TeVEarly2022Collision --step GEN,SIM --geometry DB:Extended --customise_commands process.RandomNumberGeneratorService.generator.initialSeed="int(${SEED})"\\nprocess.source.numberEventsInLuminosityBlock="cms.untracked.uint32(${NEVENTLUMIBLOCK})" --conditions $GLOBALTAG --datatier GEN-SIM --eventcontent RAWSIM --python_filename GS_cfg.py --fileout file:gensim.root --mc --nThreads $NTHREAD -n $NEVENT || exit $? ;
