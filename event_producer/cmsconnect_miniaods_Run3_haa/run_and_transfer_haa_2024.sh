...
cmsDriver.py Configuration/GenProduction/python/${PROCNAME}.py --era Run3_2024 --customise Configuration/DataProcessing/Utils.addMonitoring --beamspot DBrealistic --step GEN,SIM --geometry DB:Extended --customise_commands process.RandomNumberGeneratorService.generator.initialSeed="int(${SEED})"\\nprocess.source.numberEventsInLuminosityBlock="cms.untracked.uint32(${NEVENTLUMIBLOCK})" --conditions $GLOBALTAG --datatier GEN-SIM --eventcontent RAWSIM --python_filename GS_cfg.py --fileout file:gensim.root --mc --nThreads $NTHREAD -n $NEVENT || exit $? ;
...
# Change to CMSSW_15_0_4 before SKIM
